...
const WalletDashboard: React.FC<WalletDashboardProps> = ({
  ...
  } = useGetWalletDetailsQuery(walletId, { skip: !walletId });
  const {
    data: payoutData,
    isLoading: isLoadingPayouts,
    refetch: refetchPayouts,
  } = useGetPayoutAccountsQuery(walletId, { skip: !walletId });
  const { data: banksData, isLoading: isLoadingBanks } =
    useGetNigerianBanksQuery();

  // Mutations
  const [createPayoutAccount, { isLoading: isCreatingPayout }] =
    useCreatePayoutAccountMutation();
  const [verifyPayoutAccount, { isLoading: isVerifyingPayout }] =
    useVerifyPayoutAccountMutation();
  const [withdrawFunds, { isLoading: isWithdrawing }] =
    useWithdrawFundsMutation();
  const [resolveAccount, { isFetching: isResolvingAccount }] =
    useLazyResolveBankAccountQuery();
      ...
      resolveAccount({
        ...
        .catch((err: any) => {
          const errorMsg =
            ...
            err?.data?.message ||
            err?.message ||
            'Could not resolve account name. Please enter manually.';
          setAddBankError(
    ...
    try {
      ...
      if (response.data?.id) {
        try {
          ...
          console.log(
            'Auto-verify failed, user can manually verify later',
            verifyErr
          );
        }
      }
    ...
    <Box>
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} md={8}>
              ...
              {(parseFloat(wallet?.pending_cash || '0') > 0 ||
                parseFloat((wallet as any)?.pendingCash || '0') > 0) && (
                <Typography variant="body2" sx={{ opacity: 0.8 }}>
                  Pending/Locked:{' '}
                  {formatCurrency(
                    wallet?.pending_cash || (wallet as any)?.pendingCash,
                    wallet?.currency
                  )}
                </Typography>
              )}
      ...
      <StyledCard>
        <List sx={{ p: 0 }}>
          {isLoadingPayouts ? (
            <ListItem>
              <CircularProgress size={24} sx={{ my: 2, mx: 'auto' }} />
            </ListItem>
      ...
      >
        ...
        <DialogContent>
          ...
          <TextField
            ...
            InputProps={{
              endAdornment: isResolvingAccount && (
          ...
          >
            {payoutAccounts.map((acc: any, index: number) => (
              <MenuItem
                key={`${acc.id}-${index}`}
                value={acc.id}
                disabled={!acc.is_verified}
              >
                {acc.bank_name} - {acc.account_number}{' '}
                {!acc.is_verified && '(Unverified)'}
              </MenuItem>
            ))}
            {payoutAccounts.length === 0 && (
