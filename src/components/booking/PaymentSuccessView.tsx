...
import PayoutNudgeModal from './PayoutNudgeModal';
import {
          ...
          <div className="flex flex-col items-center justify-center p-6 printable-section">
            ...
            <div className="mt-4 text-center">
              ...
              <h2 className="text-[20px] font-medium">
                {formatPrice(booking?.total_charging_fee ?? 0)}
              </h2>
            </div>
            ...
            <div className="mt-6 w-full max-w-xl sm:w-full border rounded-lg bg-white shadow-md">
              ...
              <div className="flex justify-between items-center mb-4 px-4 space-x-14">
                <p className="text-black font-medium text-[13px]">
                  {formatPrice(booking?.base_price ?? 0)} x {booking?.nights}{' '}
                  nights x {booking?.unit_count} unit
                  {booking?.unit_count !== 1 ? 's' : ''}
                </p>
                <p className="text-gray-500 text-[13px]">
                  Total(NGN) {formatPrice(booking?.total_charging_fee ?? 0)}
                </p>
              </div>
