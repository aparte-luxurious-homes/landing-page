...
const ListFlow7: React.FC<ListFlow7Props> = ({ onNext, onBack, formData, setFormData }) => {
  ...
  // Initialize media from formData if available
  useEffect(() => {
    if (formData?.media) {
      setMedia(formData.media);
    }
    if (formData?.coverIndex !== undefined) {
      setCoverIndex(formData.coverIndex);
    }
  }, [formData]);
